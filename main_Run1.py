    ...
    def fill_attendance():
        ...
        if subb == '':
            ...
            def create_csv():
                ...
                base_path = os.path.join("Attendance", "Manually Attendance")
                if not os.path.exists(base_path):
                       os.makedirs(base_path)
                csv_name = os.path.join(base_path, DB_table_name + ".csv")
                fileName = "Attendance/" + subb + "_" + Date + "_" + Hour + "-" + Minute + "-" + Second + ".csv"
            ...
            def attf():
                attendance_path = os.path.join("Attendance", "Manually Attendance")
                import subprocess

                if os.path.exists(attendance_path):
                      subprocess.Popen(r'explorer /select,"' + attendance_path + '"')
                else:
    ...
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                root.configure(background='grey80')
                import os
                base_path = r'C:\Users\M.SHIVA KUMAR\Documents\projects\edunet internship\attendence management system'
                cs = os.path.join(base_path, fileName)
    ...
    def log_in():
        ...
        if username == 'pragya':
            if password == 'pragya123':
                ...
                import os
                base_path = r'C:\Users\M.SHIVA KUMAR\Documents\projects\edunet internship\attendence management system'
                cs = os.path.join(base_path, 'StudentDetails', 'StudentDetails1.csv')
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
